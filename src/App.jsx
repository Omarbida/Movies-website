...
function App() {
  ...
  useEffect(()=>{
    fetch('https://yts.mx/api/v2/list_movies.json?limit=50').then((res)=>{
        res.json().then((data)=>{
...
const fillter = ()=>{
  const tempMovies = movies.filter((movie,i)=>{
    return ((movie.title.toLowerCase().includes(searchValue.toLowerCase()) || searchValue == '')&&
...
useEffect(()=>{
  ...
  return (
    <div className="App">
      <Head fillter={fillter} filterHandler={getFilterHandler} onchange={searchValueHandler}></Head>
      <div className="content">
        <div className={showInfo? "movies sided" : 'movies'}>
